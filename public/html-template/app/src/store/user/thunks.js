...
export const updateProfileData = ({country, city, address, phoneNumber, email, token, backup_email, state, position}) => {
    ...
    return async(dispatch) => {
            ...
            .then( res => {
                dispatch(setChangeLoading({ ok: false }))
                if (!res.error) {
                    dispatch( updateUserData({ email: res.data.email, birthDate: res.data.birth_date, address: res.data.address, phone: res.data.phone, ciudad: res.data.city, country_id: res.data.country_id, country_name: res.data.country_name, state: res.data.state }) )
                    dispatch( setChangeOk({ok: true}) )
