...
export const Dashboard = () => {
  ...
  return (
    <div className='pt-24 min-h-[100vh] relative lg:static items-center lg:px-40 xl:px-52 px-8 flex flex-col' >
        ...
        <div className='pt-5 duration-700 w-full min-h-[36.5rem]' >
            { apps.length? (apps.map( (el, i) => {
                 if(i === 0) {
                  ...
                  return <DasboardItem logo={el.logo} secret={el.secret} status={el.status} title={el.title}  token={el.token} key={el.id} id={el.id}  appurls={el.appurls} fabricante={el.fabricante} website={el.website} />
                     ...
                     <div className='shadow-custom rounded-[5px] flex justify-center items-center h-[5rem] w-full bg-white mt-16 dark:bg-darkmode-black-02 ' >
